...
Camera::Camera(int argc, char** argv)
  ...
  , _fd(-1) {
	...
	if (cmdOptionExists(argc, argv, "--verbose")) _verbose = true;
...
Camera::setResolution(uint32_t w, uint32_t h) {
	...
	if (-1 == ::ioctl(_fd, VIDIOC_S_FMT, &fmt)) {
		std::cerr << "(ERROR) " << _device << ": VIDIOC_S_FMT ("
		          << std::to_string(_IOC_NR(VIDIOC_S_FMT)) << "): " << std::strerror(errno)
		          << std::endl;
...
Camera::setFrameRate(std::uint32_t frame) {
	...
	if (-1 == ::ioctl(_fd, VIDIOC_S_PARM, &parm)) {
		std::cerr << "(ERROR) " << _device << ": VIDIOC_S_PARM ("
		          << std::to_string(_IOC_NR(VIDIOC_S_PARM)) << "): " << std::strerror(errno)
		          << std::endl;
...
Camera::lockKernelBuffer() {
	...
	// wait unread image
	{
		...
		if (0 == r) {
			throw std::runtime_error(_device +
			                         ": Can't get new image from driver: timeout on select");
		} else if (-1 == r) {
...
Camera::getImage() {
	...
	std::memcpy(result.get(), _v4l2Buffers[buffer.index].first, _width * _height * 2);
...
void
CameraLIOV5640::setAutoExposure(bool enable) {
	...
	if (_verbose) {
		std::cout << "(VERBOSE) " << _device << ": set AutoExposure to " << enable
		          << std::endl;
	}
...
void
CameraLIOV5640::setExposure(std::int32_t value) {
	struct v4l2_control control;
	::memset(&control, 0, sizeof(control));
	...
	if (_verbose) {
		std::cout << "(VERBOSE) " << _device << ": set Exposure to " << value << std::endl;
	}
	if (-1 == ::ioctl(_fd, VIDIOC_S_CTRL, &control)) {
		throw std::runtime_error(_device + ": setExposure fails: " + std::strerror(errno));
	}
}

void
CameraLIOV5640::setGain(std::int32_t value) {
	struct v4l2_control control;
	::memset(&control, 0, sizeof(control));
	...
	if (_verbose) {
		std::cout << "(VERBOSE) " << _device << ": set Gain to " << value << std::endl;
	}
	if (-1 == ::ioctl(_fd, VIDIOC_S_CTRL, &control)) {
		throw std::runtime_error(_device + ": setGain fails: " + std::strerror(errno));
	}
}
